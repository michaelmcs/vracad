...
<head>
    ...
    <title>Tabla de Anexo 7</title>
</head>
<body>
    <div class="heading">
    <h1>Carga Acadecima Universidad Nacional Del altiplano</h1>
  </div>
    ...
    <table >
        <thead>
			...
			<th>Categoria</th>
			<th>Dedicacion</th>
			<th>escuela la que brinda servicios</th>
			<th>total de Hrs Carga Acad.</th>
			<th>Observacion</th>
        ...
        <tbody >
				...
				$query=mysqli_query($con,"SELECT DISTINCT b.member_id, c.prog_id , a.subject_code , c.nombre, c.subject_title, a.cys,	c.tipo_cur, a.sec, d.turno, a.room,  a.alum, a.hteo, a.hprac,
				...
				while($row=mysqli_fetch_array($query)){
					  ...
					  if( $b === 0 ){
						$docente = $row['member_id'];
						$query6=mysqli_query($con,"SELECT COUNT( distinct a.subject_code, c.subject_title,  a.sec ) AS cont FROM schedule a 
						
						INNER JOIN subject c ON a.subject_code = c.subject_code
						INNER JOIN member d ON a.member_id = d.member_id 
					    Where c.prog_id AND d.member_id = $docente ") or die(mysqli_error());
				...
				if ($prog==$mid or $prog==44 or $prog == $id)  
					...
					?>
					<tr>
					<td class="first"><?php echo $contt ?></td> <!-- Codigo del curso -->
					...
					<!-- Total de credito -->
					 ...
					 <?php if($b==0){?>
						...
						<td rowspan ="<?php echo $rowd + $contG +1 ?>" ><?php echo $member_first, ", ", $member_last?></td>
